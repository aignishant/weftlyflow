"""Google Drive OAuth2 credential — pre-filled for the Drive API.

Identical wire contract to :class:`OAuth2GenericCredential` — the
access token is injected as ``Authorization: Bearer <token>`` — but the
...
_DEFAULT_SCOPE: str = "https://www.googleapis.com/auth/drive.file"


class GoogleDriveOAuth2Credential(OAuth2GenericCredential):
    """Google-flavoured OAuth2 credential scoped for the Drive API."""

    slug: ClassVar[str] = "weftlyflow.google_drive_oauth2"
    display_name: ClassVar[str] = "Google Drive OAuth2"
    ...
    properties: ClassVar[list[PropertySchema]] = [
        ...
        PropertySchema(
            ...
            required=False,
            description="Space-separated Drive API scopes.",
        ),
        PropertySchema(
            name="access_token",
...
TYPE = GoogleDriveOAuth2Credential
