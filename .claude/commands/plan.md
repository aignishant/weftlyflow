...
1. **Goal** — one sentence.
2. **Context** — what's already in the codebase; which spec section applies.
...
8. **Docs** — what gets updated in `docs/`; whether the spec needs a revision-log entry.
...
- If a chosen approach conflicts with the spec, say so and propose a spec amendment in the same plan.
