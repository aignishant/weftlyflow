...
- [x] External-secrets provider interface (`weftlyflow.credentials.external`) — `SecretProvider` protocol + `EnvSecretProvider` + `SecretProviderRegistry` + `VaultSecretProvider` (KV v2, token auth) + `OnePasswordSecretProvider` (Connect REST, bearer token) + `AWSSecretsManagerProvider` (boto3, default credential chain, gated behind the `aws-secrets` optional extra).
...
- [x] SAML/OIDC SSO — `weftlyflow.auth.sso` (`OIDCAdapter`, `SAMLAdapter` with signed `AuthnRequest`, `StateToken` CSRF guard, `NonceStore` protocol + in-process + Redis-backed implementations for multi-instance replay protection); operator runbook in `docs/guide/single-sign-on.md`.
...
| 2026-04-24 | Phase 8b closeout | SSO shipped — OIDC + SAML 2.0 adapters under `weftlyflow.auth.sso` with signed SAML `AuthnRequest`, `StateToken` CSRF guard, and a `NonceStore` protocol (in-process + Redis-backed) for replay protection across multi-instance deploys; operator runbook at `docs/guide/single-sign-on.md`. Phase 8b now fully delivered. |
